...
class TestTicTacToeEngine(TestCase):
    ...
    def test_proper_initialization(self):
        ...
        self.assertTrue(self.engine.allowed_actions == list(product(range(self.board_size), range(self.board_size))))
        ...
        # self.assertFalse(self.engine.moves_made)
    ...
    def test_make_move(self):
        self.engine.make_move(0, 0)  # Player 0
        self.assertTrue(self.engine.current_board[0][0] == 0)
        self.assertTrue(self.engine.current_player == Player("Player 1", 1))
        self.assertFalse(self.engine.winnings)

        self.engine.make_move(0, 1)  # Player 1
        self.assertTrue(self.engine.current_board[0][1] == 1)
        self.assertTrue(self.engine.current_player == Player("Player 2", 2))
        self.assertFalse(self.engine.winnings)

        self.engine.make_move(3, 3)  # Player 2
        self.assertTrue(self.engine.current_board[3][3] == 2)
        self.assertTrue(self.engine.current_player == Player("Player 0", 0))
        self.assertFalse(self.engine.winnings)
    ...
    def test_make_move_fail(self):
        ...
        self.assertRaises(IllegalMoveError, self.engine.make_move, 2, 3)
        self.assertRaises(IndexError, self.engine.make_move, 5, 5)
    ...
    def test_allowed_actions_with_no_actions(self):
        ...
        self.assertFalse(set(self.engine.allowed_actions))
