...
3. Run cell below and next choose Kernel-> Change kernel -> Python3.7-WinTacToe
4. Refresh jupyter notebook page in your browser
...
class MonteCarloES:
    ...
    def gen_episode(self, agent1, agent2, epsilon):
        ...
        env = agent1.env
        players = [agent1.player, agent1.player]
        size = 3
        marks_required = 3
        env.initialize(players, size, marks_required)
...
# Plot 
x = list(range(len(mces.agent1_G)))
y = mces.agent1_G
plt.scatter(x, y)
...
x = np.array(mces.agent1_G)[-1000:-1]
...
Nauka agenta przy pomocy podstawowej metody monte carlo jest efektywna tylko gdy drugi gracz ma stałą taktykę (nie uczy się). Za stałą taktykę uważamy również taktykę w pełni losową.
